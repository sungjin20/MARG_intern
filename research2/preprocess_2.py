...
# (9) 이미지 생성
all_images = {}  # {(gu, year_month): np.array}

for idx, row in df.iterrows():
    address = f"부산광역시 {row['Gu']} {row['Address']}"
    start = row['Start']
    months = generate_months(start)
    
    lat, lon = get_coords(address)
    if lat is None or lon is None:
        print(f"⚠️ 위경도 없음: {address}")
        continue

    for gu, bounds in gu_center_bounds.items():
        lat_min, lat_max, lon_min, lon_max = bounds
        if lat_min <= lat <= lat_max and lon_min <= lon <= lon_max:
            x, y = coord_to_pixel(lat, lon, bounds, IMAGE_SIZE)
            for ym in months:
                key = (gu, ym)
                if key not in all_images:
                    all_images[key] = np.zeros(IMAGE_SIZE, dtype=np.uint8)
                all_images[key][y, x] = 255  # 흰 점으로 표시

# (10) 이미지 저장
for (gu, ym), img in all_images.items():
    save_dir = f"/data2/personal/sungjin/research2/output/{gu}"
    os.makedirs(save_dir, exist_ok=True)
    Image.fromarray(img).save(f"{save_dir}/{ym}.png")

# (11) 모델 학습용 데이터 저장 (.npy 형식)
npy_save_dir = "/data2/personal/sungjin/research2/npy_data"
os.makedirs(npy_save_dir, exist_ok=True)

for (gu, ym), img in all_images.items():
    binary_img = (img > 0).astype(np.uint8)  # 흰 점이 있는 곳은 1, 나머지는 0
    np.save(f"{npy_save_dir}/{gu}_{ym}.npy", binary_img)
