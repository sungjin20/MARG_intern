...
class FocalLoss(nn.Module):
    def __init__(self, alpha=0.25, gamma=2.0, reduction='mean'):
        """
        :param alpha: 희귀 클래스(양성 클래스)의 가중치
        :param gamma: 난이도 조절 인자 (높을수록 쉬운 샘플의 기여가 줄어듦)
    ...
    def forward(self, inputs, targets):
        """
        :param inputs: 예측값 (logits 또는 확률), shape (B, 1, H, W)
        :param targets: 정답 mask, shape (B, 1, H, W)
        """
        # Sigmoid 적용 후 clipping (안정성 확보)
...
# 👇 여기서 criterion을 BCE+Dice Loss로 변경
