...
checkpoint_save_dir = '/data2/personal/sungjin/korean_dialects/classification_third_checkpoint'
    ...
    def __init__(self, dialect_data_dir, standard_data_dir, regions, mode='train', transform=None):
        ...
        for region in regions:
            ...
            if region == 'standard':
                ...
                base_dir = os.path.join(standard_data_dir, 'phoneme')
            else:
            ...
            for line in lines:
                self.data.append((region, line.strip(), label))
    
    def __len__(self):
        return len(self.data)
    
    def __getitem__(self, idx):
        region, line, label = self.data[idx]

        base_dir = os.path.join(standard_data_dir, 'phoneme')
        phoneme_path = os.path.join(base_dir, line).replace(".wav", ".npy")
        ids = torch.from_numpy(np.load(phoneme_path))

        return ids, label

# Collate Function 정의
def collate_fn(batch):
    idss, labels = zip(*batch)

    lengths = torch.tensor([ids.shape[0] for ids in idss], dtype=torch.long)
    max_len = max(lengths)

    padded_idss = torch.stack([
        F.pad(ids, (0, max_len - ids.shape[0]), "constant", 0) for ids in idss
    ])

    # Label 텐서 변환
    labels = torch.tensor(labels, dtype=torch.long)

    return padded_idss, labels
...
# TensorBoard 설정
writer = SummaryWriter(log_dir='/data2/personal/sungjin/korean_dialects/classification_tensorboard_logs_third')

# 로그 설정
logging.basicConfig(
    level=logging.INFO,  # 로그 레벨 설정
    format='%(asctime)s - %(message)s',  # 로그 포맷 설정
    handlers=[logging.FileHandler('classification_train_log_third.txt'), logging.StreamHandler()]  # 로그를 파일과 콘솔로 출력
)
